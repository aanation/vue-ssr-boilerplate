...
* **BACKEND_URL** - на этот URL будет обращаться модуль API (/source/app/api). В виду того, что замена этой переменной предполагается не часто, она вынесена в app.env файл в корне проекта и передается вебпак конфигу через [dotenv](https://www.npmjs.com/package/dotenv)
